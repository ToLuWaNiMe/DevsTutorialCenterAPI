...
namespace DevsTutorialCenterAPI.Utilities
{
    ...
    {
        public static PaginatorResponseDto<IEnumerable<T>> Paginate<T>(IEnumerable<T> items, int pageNum, int pageSize)
        {
            ...
            var paginatedItems = items.Skip(skipAmount).Take(pageSize);
